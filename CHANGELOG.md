...
### `2.2.0` — `vpts.analysis`: financial time-series diagnostics + point-in-time fundamental data
A separate part covering the two data-analysis families the repo did not have: **time-series diagnostics** and **fundamental data**. Both are wired into the existing evaluation contract, and neither is allowed to claim an edge on its own.
- **Added** `vpts.analysis.timeseries` — `analyze_timeseries() → TimeSeriesReport` plus every estimator standalone: annualised return/vol/Sharpe, distribution (skew, excess kurtosis, Jarque–Bera), tails (VaR/CVaR, MAD-scaled jump fraction), memory (`autocorrelation`, `ljung_box`, **Lo–MacKinlay `variance_ratio`** with the heteroskedasticity-robust z, **Anis–Lloyd-corrected `hurst_exponent`**, `adf_test`), volatility structure (`arch_lm_test`, `parkinson_vol`, `garman_klass_vol`) and `drawdown_stats` (max, longest underwater run, ulcer index). Every statistic is **in-sample by construction** and `summary()` says so on its last line — this half describes, it does not claim.
- **Added** `vpts.analysis.fundamentals` — a provider-agnostic `FundamentalsSource` mirroring `DataSource`: `FMPFundamentalsSource` (joins income/balance/cash-flow on the fiscal period, takes `available_at` from FMP's real `filingDate`/`acceptedDate`, injectable `http_get`, surfaces the 402/403 plan gate) and `SyntheticFundamentalsSource` (offline, deterministic, annual or quarterly, optional planted signal link). `compute_ratios` → 17 `FUNDAMENTAL_FEATURES` (valuation expressed as **yields**, so they stay finite through zero earnings) plus `piotroski_f_score` (0–9) and `altman_z_score`.
- **Added** the point-in-time contract, enforced at three levels: `FundamentalSnapshot` **raises** if `available_at < period_end`; `align_asof`/`align_fundamentals` do a backward as-of merge on the *filing* date and never back-fill; `audit_point_in_time` re-checks every bar and `fundamental_feature_frame` **raises** rather than emit a leaked matrix. Joining on the period end would leak the ~75-day reporting lag into every row.
- **Added** `vpts.analysis.dataset` — `build_fundamental_dataset` (→ `FactorDataset`), `build_combined_dataset` (structural ⊕ fundamental, for the nested "does accounting data add to microstructure?" comparison) and `build_fundamental_panel` (→ `CrossSectionalPanel`, reusing `ml.cross_sectional`'s ranking). Fundamentals are judged by exactly the same harness as everything else.
- **Found (honesty):** daily sampling of fundamental features **breaks the significance test**. `recommend_block_size` sizes the permutation block from the *label* horizon (3 samples at `horizon=20, stride=10`) while the *feature* is constant across ~25 consecutive samples, so the null is far too tight. On fundamentals generated **independently of price**, daily sampling reported **IC +0.16, p = 0.005** — a confident verdict on pure noise. `build_fundamental_dataset` therefore emits **one row per filing** by default and derives `stride` from the realised bar gap; the null then clears.
- **Found (honesty):** that honest sampling puts fundamentals *by construction* below the harness's `MIN_SAMPLES_FOR_PERM = 120` (~40 rows/name), where `honest_backtest`'s convenience p-value is documented-unreliable — measured **2/12 false-positive at the 5% level**. The **cross-sectional panel** is both the financially correct frame and the statistically sound one: pooling by date gives ~20× the effective sample and a within-date shuffle, and it false-positives at ~the nominal rate (**1/10 measured**) while still detecting a planted edge (IC +0.10, p = 0.02). `examples/act5_analysis_demo.py` prints both and says which to trust.
- **Fixed** two estimator bugs caught by the null-clearing tests before release: a spurious `√n` in the variance-ratio z-statistic (a random walk scored z ≈ +10 instead of ≈ +0.2), and the uncorrected R/S Hurst returning ≈0.57 on i.i.d. data (now Anis–Lloyd corrected → ≈0.50).
- **Added** `examples/act5_analysis_demo.py` (offline; `--plant-edge` to confirm the harness still finds a real edge) and 41 tests — every estimator against a known answer, a truncate-the-future no-look-ahead check, an offline FMP parser, and the mandatory signal/null-clearing pair. Suite: **318 → 359**.
...
- **Found & guarded (honesty):** validating the above surfaced that `honest_backtest`'s block-permutation p **over-rejects true i.i.d. nulls in a small-sample regime** — ≈⅓ false-positive (0.33 over a 40-trial recheck) at ~48 samples/dataset, **~6× the 5% nominal**, because the small-sample OOS IC is biased/unstable; it returns to near-nominal (≈7%) at ~170+ samples/dataset. Added `MIN_SAMPLES_FOR_PERM` (120): below it, a "significant" result is flagged as **unreliable** with a warning. **This does *not* affect the eleven published experiments** — those use the standalone, review-verified `block_permutation_test` on the full 88-name / ~1,300-bar sample, not this convenience path.
